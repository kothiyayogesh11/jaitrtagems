...
class Profile_model extends Data {
	var $tbl_client_master = "client_master";
	var $tbl_user_master = "user_master";
    function __construct(){
        parent::__construct();
        $this->tbl = $this->tbl_user_master;
		$this->alias = "c";
    }
	...
	function pets_type_list(){
        ...
        if(isset($searchCriteria['not_id']) && $searchCriteria['not_id'] != ""){
            $whereClaue .= 	" AND ".$this->alias.".user_id !=".$searchCriteria['not_id']." ";
        }
	...
	function check_client_email($email = NULL, $id = NULL){
		$where = "1 = 1";
		if($email != "") $this->db->where(array("user_email"=>$email));
	...
	function delete_by_id($id = NULL){
		return $this->db->where(array("user_id"=>$id))->delete($this->tbl);
	}
}