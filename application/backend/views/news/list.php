...
?>
			...
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">News - List <!--<small>header small text goes here...</small>--></h1>
			<!-- end page-header -->
            <!-- begin nav-pills -->
			<p class="m-b-0">
                <a href="<?php echo base_url("news/form"); ?>" class="btn btn-white m-r-5"><i class="fa fa-fw m-r-10 pull-left f-s-18 fa-plus"></i> Add</a>
                ...
                <div class="panel pagination-inverse clearfix m-b-0">
                    ...
                    <table id="data-table" data-order='[[2,"asc"]]' class="table table-bordered table-hover">
                        ...
                        <tr class="<?php echo $row;?>" id="row_<?php echo $rec["id"]; ?>" data-val="<?php echo $rec["id"]; ?>">
                            ...
                            <td><a href="<?php echo base_url("news/form/".$rec["id"]) ?>"><?php echo $rec["title"]; ?></a></td>
                            <td><?php echo $rec["content"]; ?></td>
                            <td><?php echo date("D, d-M, Y",strtotime($rec["date"])); ?></td>
...
function DeleteRow(){
	...
	if(intChecked == 0){
		...
		if(responce==true){
			$('#frm_add').attr('action',BASE_URL+'news/delete');
			$('#frm_add').submit()	
		}
	}
}
