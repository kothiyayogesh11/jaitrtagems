...
?>
			...
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">Slot - List <!--<small>header small text goes here...</small>--></h1>
			<!-- end page-header -->
            <!-- begin nav-pills -->
			<p class="m-b-0">
                <a href="<?php echo base_url("slot/form"); ?>" class="btn btn-white m-r-5"><i class="fa fa-fw m-r-10 pull-left f-s-18 fa-plus"></i> Add</a>
			...
			<div class="section-container section-with-top-border">
                ...
                <div class="panel pagination-inverse clearfix m-b-0">
                    ...
                    <table id="data-table" data-order='[[1,"asc","asc","asc","asc"]]' class="table table-bordered table-hover">
                        <thead>
                            <tr class="">
                        ...
                        <tr class="<?php echo $row;?>" id="row_<?php echo $rec["id"]; ?>" data-val="<?php echo $rec["id"]; ?>">
                            ...
                            <td><?php echo $i+1; ?></td>
                            <td><a href="<?php echo base_url("slot/form/".$rec["id"]) ?>"><?php echo $rec["activity_title"]; ?></a></td>
                            <td><?php echo $rec["date"] != "" ? date("D d-M, Y",strtotime($rec["date"])) : "--"; ?></td>
                            <td><?php echo date("h:i A",strtotime($rec["from_time"])).' to '.date("h:i A",strtotime($rec["to_time"])); ?></td>
...
function DeleteRow(){
	...
	if(intChecked == 0){
		...
		if(responce==true){
			$('#frm_add').attr('action',BASE_URL+'slot/delete');
			$('#frm_add').submit()	
		}
	}
}
