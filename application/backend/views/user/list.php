...
echo form_open_multipart('banner', $attributes);
?>
			<!-- begin breadcrumb -->
			...
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">User - List <!--<small>header small text goes here...</small>--></h1>
			<!-- end page-header -->
            <!-- begin nav-pills -->
			<p class="m-b-0">
                <a href="<?php echo base_url("user/AddUser"); ?>" class="btn btn-white m-r-5"><i class="fa fa-fw m-r-10 pull-left f-s-18 fa-plus"></i> Add</a>
			...
			<div class="section-container section-with-top-border">
                ...
                <div class="panel pagination-inverse clearfix m-b-0">
                    ...
                    <table id="data-table" data-order='[[1,"asc"]]' class="table table-bordered table-hover">
                        ...
                        ?>
                        <tr class="<?php echo $row;?>" id="row_<?php echo $rec["user_id"]; ?>" data-val="<?php echo $rec["user_id"]; ?>">
                            <td><input type="checkbox" name="record[]" value="<?php echo $rec["user_id"]; ?>" /></td>
                            <td><a href="<?php echo base_url("user/AddUser/".$rec["user_id"]) ?>"><?php echo $rec["u_typ_name"]; ?></a></td>
                            <td><?php echo $rec['user_full_name']; ?></td>
                            <?php 
							if(isset($rec['parent_user_id']) && $rec['parent_user_id'] != '' && $rec['parent_user_id'] != NULL){
								$query = $this->db->get_where('user_master', array('user_id' => $rec['parent_user_id']), 1);
								$row = $query->row();
								echo @$row->name;
...
function DeleteRow(){
	...
	if(intChecked == 0){
		...
		if(responce==true){
			$('#frm_add').attr('action',BASE_URL+'user/delete');
			$('#frm_add').submit()	
		}
	}
}
