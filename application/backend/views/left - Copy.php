...
$page_controller = @$uriArr[1];
$page_url = "";
if(isset($uriArr[1])) $page_url = $uriArr[1];
if(isset($uriArr[2])) $page_url .= '/'.$uriArr[2];
...
$currPanelId = isset($rsModule[0]['panel_id']) ? $rsModule[0]['panel_id'] : NULL;
?>
    ...
    <ul class="nav nav-list">
		...
		<?php
			...
			foreach($rsPanels AS $panel)
			{
				...
				if(isset($moduleArr[$panel['panel_id']]) && count($moduleArr[$panel['panel_id']]) > 0)
				{
					...
					foreach($moduleArr[$panel['panel_id']] AS $module)
					{
                        ...
                        <?php
						echo'	 <a href="'.base_url($module['module_url']).'" style="width:130px;">
