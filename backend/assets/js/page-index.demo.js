...
var handleWidgetReload = function() {
    ...
    $(document).on('click', '[data-click="widget-reload"]', function(e) {
		...
		if($type == "user-count"){
			$url = "count_user";
		}else if($type == "pets-count"){
			$url = "count_pets";
		}else if($type == "pending-order-count"){
			$url = "count_pending_order";
		}else if($type == "order-count"){
			$url = "count_order";
		}
		...
		if($url != ""){
			$.ajax({
				...
				beforeSend: function(){handleLoader(targetWidget,true);},
				dataType:"json",
				success:function(res){
					$obj.parents(".widget").find(".widget-stat-number").text(res.totol);
					$obj.parents(".widget").find(".widget-stat-text").text(res.week);
					handleLoader(targetWidget,false);
				}
			});
		}else{
			var targetWidget = $(this).closest('.widget');
...
var handleLoader = function(targetWidget,loader_handler){
	"use strict";
	if(loader_handler){
		$(targetWidget).append('<div class="widget-loader"><span class="spinner-small">Loading...</span></div>');
	}else{
		setTimeout(function() {	$(targetWidget).find('.widget-loader').remove();}, 500);
	}
}
...
var PageDemo = function () {
	...
	return {
		...
		init: function () {
			...
			if($('[data-click="widget-reload"]').length > 0){
				$('[data-click="widget-reload"]').each(function(index, element) {
                   $(element).click();
                });
			}
		}
